...
class ModelTrainer:
    def __init__(self, input_dim, num_classes):
        self.model = CNN(input_dim, num_classes).to(device)
...
# Example usage
def run_maincnn():
    data_processor = DataPreprocessor('train.csv', 'val.csv', 'test.csv')
    train_loader, val_loader, test_loader, num_classes = data_processor.preprocess_data(*data_processor.load_data())

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_trainer = ModelTrainer(train_loader.dataset.tensors[0].shape[1], num_classes)
